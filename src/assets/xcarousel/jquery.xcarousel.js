...
//				.trigger('mouseleave');
			...
			function init(){
				...
				for(var i=1;i<=len;i++){
					...
					// 点击页码切换
					$txtUl = $(".homemain .scroll_txt").children("ul");
					$txtUl.on('mouseenter',"li",function(){
						index = $(this).index();
						showPic();
					})
					//span点击
			...
			function showPic(){
				...
				if(opt.page) {
					...
					var $txtSpan = $txtUl.children().removeClass().eq(index).children("span");
					$txtSpan.css("background","#fff");
					$txtUl.children().removeClass().eq(index).siblings().children("span").css("background","#2997b7");
					$txtUl.children().removeClass().eq(index).children("h1").css("color","#fff");
					$txtUl.children().removeClass().eq(index).siblings().children("h1").css("color","#108db1");
					$txtUl.children().removeClass().eq(index).children("p").css("color","#fff");
					$txtUl.children().removeClass().eq(index).siblings().children("p").css("color","#3399cc");
				}
			}
		})
